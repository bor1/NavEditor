...
$permissions = NavTools::ifsetor($_POST['permissions'], '');
    ...
    if (count($files) > 2) { /* The 2 accounts for . and .. */
        ...
        foreach ($files as $file) {
            if (file_exists($root . $dir . $file) && $file != '.' && $file != '..' && is_dir($root . $dir . $file)) {
                ...
                $chkHtml = ($permissions) ? "<input class='check_box' type='checkbox' value='" . $relation . "/'>" : "";
        ...
        $html .= "</ul>";
        //special fuer rechte
        $html = (($permissions && $dir == "/") ? "<ul class='jqueryFileTree'><li class='directory expanded'><p rel='/' href='#'>Alle Seiten</p><input class='check_box' type='checkbox' value='/'>" : "" ) . $html;
    }
    $json_data = array(
        'html' => $html,
        'filesinfo' => $filesinfo
