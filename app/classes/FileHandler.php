...
<<<<<<< HEAD
    ...
    public function __construct() {
        ...
        $this->_templateFilePath = $ne_config_info['app_path'] . 'data/templates/seitenvorlage.html';
        if(!file_exists($this->_templateFilePath)) {
            $this->_templateFilePath = $ne_config_info['app_path'] . 'data/templates/_seitenvorlage.html'; // the fail-safe
        }
        $this->_logger = null;
        ...
        $this->_navindex_backup_dir = $ne_config_info['navindex_backup_dir'];
...
//        fclose($fh);
    ...
    public function UpdateExistedPageLogos($logo_content_json, $dir, $filesuffix   ) {
        global $ne_config_info;
          ...
          if(!isset($filesuffix )) {
              $filesuffix =  $ne_config_info['defaulthtml_filesuffix'];
          }
...
//                                          && $file != 'vkapp' && $file != 'vkdaten' && $file != 'xampp') {
                    if (!in_array($file, $ne_config_info['nologoupdate_dir'])) {
                        if(is_dir($dir . $file . '/')) {
                            ...
                            if((is_array($erw)) && ($erw[count($erw) - 1] == $filesuffix)) { // replace all the .shtml files
                                if ($ne_config_info['show_logoupdate_allwebpages']==1) {
    ...
    public function UpdateStartPageLogo() {
        ...
        $start_page_path = $_SERVER['DOCUMENT_ROOT'] . '/'. $ne_config_info['directoryindex_file'];
